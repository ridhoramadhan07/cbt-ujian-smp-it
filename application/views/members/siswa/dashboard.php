...
>
  <div class="container">
    ...
    <p>Akses dilarang, silahkan hubungi Ibu/Bapak Guru<br> atau bisa klik tombol di bawah.</p>
    <button id="goForward">Kembali ke Ujian</button>
    <script>
      document
        ...
        <div class="col-12">
          <div class="card card-blue">
            <div class="card-header">
              <div class="card-title text-white">MENU UTAMA</div>
            </div>
            <div class="card-body">
              <div class="row">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <img
          ...
          src="<?= base_url('assets/img/siswa.png') ?>"
          alt="Alt Text"
        />
        <div class="img-push">
          <?= form_open('create', array('id' =>
          'komentar')) ?>
...
<script>
  ...
  function addComments(id, comments, append) {
    ...
    $.each(comments, function (i, v) {
      ...
      if (v.dari == "0") {
        ...
        avatar =
          v.foto != null
            ? '<img class="img-circle border" src="' +
              v.foto +
      ...
      comm +=
        '<div class="media mt-1" id="parent-reply' +
        v.id_comment +
        '">' +
        avatar +
        '    <div class="w-100 ml-2">' +
        '        <div class="media-body border pl-3 bg-light" style="border-radius: 20px">' +
        '            <span class="text-xs text-muted"><b>' +
        dari +
        "</b></span>" +
        '            <div class="comment-text pb-1">' +
        v.text +
        "</div>" +
        "        </div>" +
        '        <div class="ml-2">' +
        '            <span class="btn-sm mr-2 text-muted">' +
        createTime(v.tanggal) +
        "</span>" +
        '            <span id="trigger-reply' +
        v.id_comment +
        '" class="btn btn-sm mr-2 text-muted action-collapse" data-toggle="collapse" aria-expanded="true"' +
        ...
        '                  data-id="' +
        v.id_comment +
        '" data-toggle="modal" data-target="#balasanModal">' +
        '                <i class="fas fa-reply"></i> <b>Balas</b></span>';
      if (v.dari_group === "3" && v.dari === idSiswa) {
        comm +=
          '            <span class="btn btn-sm text-muted" data-id="' +
          v.id_comment +
          '">' +
      ...
      comm +=
        "        </div>" +
        '<div id="collapse-reply' +
        v.id_comment +
        '" class="p-2 collapse toggle-reply" data-id="' +
        v.id_comment +
        '" data-parent="#parent-reply' +
        v.id_comment +
        '">';
      if (v.jml != "0") {
        comm +=
          ...
          v.id_comment +
          '"></div>' +
          '<div id="loadmore-reply' +
          v.id_comment +
          '" onclick="getReplies(' +
          v.id_comment +
          ')" class="text-center mb-3 loadmore-reply">' +
          '       <div class="btn btn-default">Muat balasan lainnya ...</div>' +
          "</div>";
      }
      comm +=
        '    <div id="loading-reply' +
        v.id_comment +
    ...
    $.each(replies, function (i, v) {
      ...
      if (!sudahAda) {
        ...
        if (v.dari == "0") {
          ...
          avatar =
            v.foto != null
              ? '<img class="img-circle border" src="' +
                v.foto +
                '" alt="Img" width="35px" height="35px">'
        ...
        repl +=
          ...
          "        </div>" +
          '        <div class="ml-2">' +
          '            <small class="btn-sm mr-2 text-muted">' +
          createTime(v.tanggal) +
          "</small>";
        if (v.dari_group === "3" && v.dari === idSiswa) {
          repl +=
  ...
  function addPosts(response) {
    ...
    $.each(response, function (i, v) {
      ...
      card +=
        ...
        '    <div id="collapse-' +
        v.id_post +
        '" class="p-2 collapse toggle-comment"' +
        '         data-id="' +
        v.id_post +
        '" data-parent="#parent' +
        v.id_post +
  ...
  function getPosts() {
    ...
    setTimeout(function () {
      $.ajax({
        url:
          base_url + "siswa/getpost?halaman=" + halaman + "&kelas=" + kodeKelas,
        type: "GET",
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $.each(data, function (key, value) {
          ...
          ul +=
            '  <li class="item">' +
            '<a href="javascript:void(0)" data-toggle="modal" data-target="#pengumumanModal" data-pos="' +
            pos +
            '">' +
            '    <div class="media" style="line-height: 1">' +
            '      <img class="img-circle media-left" src="' +
            base_url +
            '/assets/img/user.jpg" width="40" height="40" />' +
        ...
        if (data.length !== 0) {
          ...
          for (let i = 0; i < jmlMapel; i++) {
            ...
            if (isIst) {
              ...
              var mpl =
                data[wktPel].nama_mapel == null
                  ? "- -"
                  : data[wktPel].nama_mapel;
              tableJadwal +=
                "<tr>" +
                '<td class="text-center">' +
                data[wktPel].jam_ke +
